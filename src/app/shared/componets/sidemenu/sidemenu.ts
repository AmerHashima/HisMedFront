...
export function switcherArrowFn() {
  ...
  fromEvent(slideLeft, 'click').subscribe(() => {
    ...
    let sideMenu: any = document.querySelector<HTMLElement>('.main-menu');
    let mainSidemenuContainer: any =
    ...
    let mainSidemenuContainerWidth = mainSidemenuContainer.offsetWidth;
    if (sideMenu.scrollWidth > mainSidemenuContainer.offsetWidth) {
      if (!document.body.classList.contains('rtl')) {
        if ( marginLeftValue < 0 && !(Math.abs(marginLeftValue) < mainSidemenuContainerWidth)) {
          sideMenu.style.marginRight = 0;
          sideMenu.style.marginLeft = Number(sideMenu.style.marginLeft.split('px')[0]) + Math.abs(mainSidemenuContainerWidth) + 'px';
        ...
        if ( marginRightValue < 0 && !(Math.abs(marginRightValue) < mainSidemenuContainerWidth)) {
          sideMenu.style.marginLeft = 0;
          sideMenu.style.marginRight = Number(sideMenu.style.marginRight.split('px')[0]) + Math.abs(mainSidemenuContainerWidth) + 'px';
          slideRight.classList.remove('d-none');
  ...
  fromEvent(slideRight, 'click').subscribe(() => {
    let sideMenu: any = document.querySelector<HTMLElement>('.main-menu');
    let mainSidemenuContainer: any = document.querySelector<HTMLElement>('.main-sidebar');
    let marginLeftValue = Math.ceil(Number(window.getComputedStyle(sideMenu).marginLeft.split('px')[0]));
    let marginRightValue = Math.ceil(Number(window.getComputedStyle(sideMenu).marginRight.split('px')[0]));
    let check = sideMenu.scrollWidth - mainSidemenuContainer.offsetWidth;
    let mainSidemenuContainerWidth = mainSidemenuContainer.offsetWidth;
    ...
    if (sideMenu.scrollWidth > mainSidemenuContainer.offsetWidth) {
      if (!document.body.classList.contains('rtl')) {
        if (Math.abs(check) > Math.abs(marginLeftValue)) {
          sideMenu.style.marginRight = 0;
          if (!(Math.abs(check) > Math.abs(marginLeftValue) + mainSidemenuContainerWidth)) {
            mainSidemenuContainerWidth = Math.abs(check) - Math.abs(marginLeftValue);
        ...
        if (Math.abs(check) > Math.abs(marginRightValue)) {
          sideMenu.style.marginLeft = 0;
          if (!(Math.abs(check) > Math.abs(marginRightValue) + mainSidemenuContainerWidth)) {
            mainSidemenuContainerWidth = Math.abs(check) - Math.abs(marginRightValue);
            slideRight.classList.add('d-none');
          }
          sideMenu.style.marginRight = Number(sideMenu.style.marginRight.split('px')[0]) - Math.abs(mainSidemenuContainerWidth) + 'px';
          slideLeft.classList.remove('d-none');
